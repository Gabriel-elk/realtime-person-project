...
default_args = {"owner": "Gabriel Carvalho",
...
def _data_stream():
    ...
    try:
        ...
        logging.info(" Log - Kafka conection sucess.")

    except Exception as e:

        # Loga qualquer erro ao tentar conectar ao broker Kafka
        logging.error(f" Log - Kafka conection Fail: {e}")
        return
    ...
    while True:
        ...
        try:
            ...
            # Obtém dados da API
            res = _data_extraction()

            # Formata os dados
            res = _format_data(res)

            # Envia os dados para o tópico Kafka
            producer.send("_kafka_topic", json.dumps(res).encode("utf-8"))

        except Exception as e:

            # Loga qualquer erro durante o streaming de dados
            logging.error(f" Log - An error has ocurred: {e}")
            continue
...
with DAG("-real-time-etl-stack",
    ...
    streaming_task = PythonOperator(task_id="_stream_from_api", 
                                    python_callable=_data_stream)
